...
public class OkGoUpdateHttpUtil implements HttpManager {
     ...
     */
    @Override
    public void asyncPost(@NonNull String url, @NonNull String json, @NonNull final Callback callBack) {
        MediaType JSON = MediaType.parse("application/json; charset=utf-8");
        RequestBody body =  RequestBody.create(JSON, json);
        OkGo.<String>post(url).upRequestBody(body).execute(new com.lzy.okgo.callback.StringCallback() {
            @Override
            public void onSuccess(com.lzy.okgo.model.Response<String> response) {
                callBack.onResponse(response.body());
            }
